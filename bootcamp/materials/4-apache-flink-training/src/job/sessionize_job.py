...
def log_session_window():
    ...
    try:
        ...
        sessionize_sink_table = create_session_events_sink_postgres(t_env)

        # Execute session window with a 5 minute gap tracking num hits per ip - host
        (
            t_env.from_path(source_table)
            .window(Session.with_gap(lit(5).minutes).on(col("window_timestamp")).alias("w"))
            # Group on host, ip and window timestamp which will define each session
            .group_by(
                col("w"),
                col("ip"),
                col("host"),
            ...
            .execute_insert(sessionize_sink_table) \
            .wait()
        )
